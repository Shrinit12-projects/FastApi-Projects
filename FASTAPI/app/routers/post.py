...
def test(db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_user), limit: int = 10, skip:int= 0, search: Optional[str] = ""):
    ...
    # result = db.query(models.Post, func.count(models.Votes.post_id).label("vote")).join(models.Votes, models.Votes.post_id == models.Post.id, isouter= True).group_by(models.Post.id).all()
    # results = []
    # for post, vote_count in result:
    #     post_details =post.__dict__
    #     post_details.pop('_sa_instance_state')
    #     results.append({
    #         "Post": post_details,  # Assuming the Post model is simple enough to be serialized like this
    #         "votes": vote_count
    #     })
    # print(results)
    return posts
